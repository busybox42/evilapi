...
function renderSslScanResult(data) {
  ...
  if (vdata.protocol_support) {
    html += `<div style="margin-top:1.2em;"><b>Protocol Support:</b> <span style="color:#333;">${Array.isArray(vdata.protocol_support) ? vdata.protocol_support.join(', ') : vdata.protocol_support}</span></div>`;
  }
  // Cipher strength
  if (typeof vdata.cipher_strength === 'number') {
    html += `<div><b>Max Cipher Strength:</b> <span style="color:#333;">${vdata.cipher_strength} bits</span></div>`;
  }
  // Certificate info
  if (vdata.cert_info) {
