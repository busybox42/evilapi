...
router.post('/generate-temp-key', async (req, res) => {
    try {
      ...
      if (existingKey) {
        return res.status(409).json({ error: `Key with name '${name}' already exists` });
      }
      ...
      if (!tempKey.publicKeyArmored || !tempKey.privateKeyArmored) {
        return logError(req, "Failed to generate armored keys", new Error("Armored keys generation failed"));
      }
      ...
      res.json({ message: "Key generated successfully", publicKey: tempKey.publicKeyArmored });
    } catch (error) {
...
router.post('/encrypt-file', upload.single('file'), async (req, res) => {
    if (req.file.size > 100 * 1024 * 1024) { // 100 MB limit
...
router.post('/decrypt-file', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        res.writeHead(200, {
            'Content-Disposition': `attachment; filename="${req.file.originalname.replace('.pgp', '')}"`,
            'Content-Type': 'application/octet-stream'
        });
